...
describe('Phase 2 — Procurement workflows (e2e)', () => {
  ...
  it('posts partial GR then remainder without over-receiving', async () => {
    ...
    const gr1Res = await api.post('/api/v1/goods-receipts').send({
      ...
      items: [{ productId: ctx.productId, storageLocationId: ctx.storageLocationId, quantity: 4, uom: 'PCS', purchaseRate: 5, expiryDate: '2030-12-31' }],
    });
    ...
    const gr2Res = await api.post('/api/v1/goods-receipts').send({
      ...
      purchaseOrderId: po.id,
      supplierName: 'E2E Supplier',
      items: [{ productId: ctx.productId, storageLocationId: ctx.storageLocationId, quantity: 6, uom: 'PCS', purchaseRate: 5, expiryDate: '2030-12-31' }],
    });
    ...
    await api
      ...
      .send({
        ...
        purchaseOrderId: po.id,
        supplierName: 'E2E Supplier',
        items: [{ productId: ctx.productId, storageLocationId: ctx.storageLocationId, quantity: 1, uom: 'PCS', purchaseRate: 5, expiryDate: '2030-12-31' }],
      })
