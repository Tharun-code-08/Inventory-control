...
export function rfqInviteSubject(content: RfqInviteEmailContent): string {
  return `RFQ ${content.rfqNumber} — ${content.rfqTitle}`;
}

export function rfqInviteText(content: RfqInviteEmailContent): string {
  const lines = [
  ...
  lines.push(
    '',
    'Open the Supplier Portal to verify your identity and submit your quote:',
    content.portalUrl,
    '',
...
export function rfqInviteHtml(content: RfqInviteEmailContent): string {
