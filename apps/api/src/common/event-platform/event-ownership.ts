...
export const EVENT_REGISTRY: EventRegistryEntry[] = [
  ...
  },
  {
    ...
    eventVersion: 1,
    ownerModule: 'email-notifications',
    producer: 'DunningScheduler.sweep',
    consumers: ['notification-engine'],
    ...
    payloadSchema: {
      ...
      daysFromDue: { type: 'number' },
      balanceDue: { type: 'number' },
      channels: { type: 'array' },
    },
    retention: 'standard',
    description: 'A dunning ladder step is due for an unpaid invoice; deliver the reminder/escalation.',
  },
  {
    eventType: 'invoice.paid',
    eventVersion: 1,
    ownerModule: 'invoices',
    producer: 'PaymentReceiptsService.record',
    consumers: ['notification-engine'],
    classification: EventClassification.FINANCIAL,
    payloadSchema: {
      invoiceId: UUID,
    ...
    retention: 'standard',
    description: 'An invoice was fully settled; resolve any active dunning thread (stop-on-payment).',
  },
  {
    eventType: 'customer.replied',
    eventVersion: 1,
    ownerModule: 'agent-platform',
    producer: 'WhatsAppWebhookController.handleInbound',
    consumers: ['notification-engine'],
    classification: EventClassification.OPERATIONAL,
    payloadSchema: {
      customerId: UUID,
      channel: { type: 'string' },
      providerMessageId: { type: 'string', optional: true },
    },
    retention: 'short-op',
    description: 'A customer replied on an outbound channel; pause active dunning (stop-on-reply).',
  },
];
