...
const product_image_storage_service_1 = require("../../common/upload/product-image-storage.service");
const product_audit_1 = require("../../common/state-machines/product-audit");
...
let ProductsService = class ProductsService {
    ...
    images;
    audit;
    constructor(prisma, stock, subscriptions, numbers, series, images, audit) {
        this.prisma = prisma;
        this.stock = stock;
    ...
    async validateAssignments(user, plants, tx = this.prisma) {
        if (plants.length === 0) {
            throw new common_1.BadRequestException('At least one plant assignment is required');
        }
        const seenShops = new Set();
        for (const plant of plants) {
            if (seenShops.has(plant.shopId)) {
                throw new common_1.BadRequestException('Duplicate plant assignment');
            }
            seenShops.add(plant.shopId);
            (0, shop_scope_1.assertShopScope)(user, plant.shopId);
            if (plant.maxStockLevel !== undefined &&
                Number(plant.maxStockLevel) < Number(plant.minStockLevel ?? 0)) {
                throw new common_1.BadRequestException('Max stock level must be greater than or equal to min stock level');
            }
            if (plant.storageLocationId) {
                ...
                if (!location || location.shopId !== plant.shopId) {
                    throw new common_1.BadRequestException('Invalid storage location for selected plant');
                }
            }
        }
    }
    plantBatchExpiryFields(plant) {
    ...
    async nextGeneratedProductCode(tx, category, reservedCodes) {
        ...
        const known = new Set([...reservedCodes, ...existing.map((row) => row.productCode)]
    ...
    async resolveGeneratedProductCode(tx, companyId, shopId, category, reservedCodes) {
    ...
    async getAccessibleShops(user, tx = this.prisma) {
        ...
        return tx.shop.findMany({
            where: tenantShopIds && tenantShopIds.length > 0
                ? { id: { in: tenantShopIds } }
                : { companyId },
    ...
    resolveShopForImportRow(user, row, shops) {
        ...
        const match = shops.find((shop) => {
            ...
            return (shop.shopNumber.toLowerCase() === lookup ||
        ...
        (0, shop_scope_1.assertShopScope)(user, match.id);
        return match;
    }
    async list(user, query) {
        const limit = query.limit ? Math.min(500, Math.max(1, query.limit)) : (0, pagination_1.clampTake)(query.limit);
        const page = query.page && query.page > 0 ? query.page : 1;
        const skip = (page - 1) * limit;
        const companyId = (0, shop_scope_1.requireCompanyId)(user);
        const shopId = query.shop_id;
        if (query.shop_id)
            await (0, shop_access_1.verifyShopInTenant)(this.prisma, user, query.shop_id);
        const tenantShopIds = (0, shop_scope_1.shopIdsForUser)(user);
        ...
        return this.prisma.$transaction(async (tx) => {
            const product = await tx.product.create({
                data: {
                    ...
                    taxPreference: dto.taxPreference ?? client_1.TaxPreference.TAXABLE,
                    gstRate: new client_1.Prisma.Decimal(dto.gstRate ?? 0),
                    ...
                    plants: {
                        create: dto.plants.map((plant) => ({
                            ...
                            openingStock: new client_1.Prisma.Decimal(plant.openingStock ?? 0),
                            ...this.plantBatchExpiryFields(plant),
                            minStockLevel: new client_1.Prisma.Decimal(plant.minStockLevel ?? 0),
                            maxStockLevel: plant.maxStockLevel === undefined ? null : new client_1.Prisma.Decimal(plant.maxStockLevel),
                            reorderQty: plant.reorderQty === undefined ? null : new client_1.Prisma.Decimal(plant.reorderQty),
            ...
            await this.audit.log((0, product_audit_1.buildCreateProductAudit)({
                companyId: (0, shop_scope_1.requireCompanyId)(user),
                userId: user.id,
                productId: product.id,
    ...
    async reorderSuggestion(user, id, shop_id) {
        const product = await this.get(user, id);
        const shopId = shop_id ?? user.shopId;
        if (!shopId) {
            throw new common_1.BadRequestException('shop_id is required when ordering for a product');
        }
        (0, shop_scope_1.assertShopScope)(user, shopId);
        const plant = product.plants.find((p) => p.shopId === shopId && p.isActive);
        if (!plant) {
            throw new common_1.BadRequestException('Product is not assigned to the selected plant');
        }
    ...
    async get(user, id) {
        ...
        const balances = await this.buildStockBalanceMap(this.prisma, [product.id], tenantShopIds && tenantShopIds.length > 0 ? tenantShopIds : undefined);
        const stockByShop = {};
        for (const [key, balance] of balances.entries()) {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            const updated = await tx.product.update({
                ...
                data: {
                    ...
                    gstRate: dto.gstRate !== undefined ? new client_1.Prisma.Decimal(dto.gstRate) : undefined,
            ...
            if (dto.plants) {
                    ...
                    if (prior) {
                        await tx.productPlant.update({
                            ...
                            data: {
                                ...
                                minStockLevel: new client_1.Prisma.Decimal(incoming.minStockLevel ?? 0),
                                maxStockLevel: incoming.maxStockLevel === undefined
                                    ? null
                                    : new client_1.Prisma.Decimal(incoming.maxStockLevel),
                                reorderQty: incoming.reorderQty === undefined
                                    ? null
                                    : new client_1.Prisma.Decimal(incoming.reorderQty),
                        ...
                        await tx.productPlant.create({
                            data: {
                                ...
                                storageLocationId: incoming.storageLocationId ?? null,
                                openingStock: new client_1.Prisma.Decimal(incoming.openingStock ?? 0),
                                ...this.plantBatchExpiryFields(incoming),
                                minStockLevel: new client_1.Prisma.Decimal(incoming.minStockLevel ?? 0),
                                maxStockLevel: incoming.maxStockLevel === undefined
                                    ? null
                ...
                for (const prior of existing.plants) {
                    if (!incomingByShop.has(prior.shopId)) {
                        const [grCount, giCount, poCount, dmgCount, ledgerCount, summaryRow] = await Promise.all([
                        ...
                        const hasHistory = grCount + giCount + poCount + dmgCount + ledgerCount > 0 ||
            ...
            if (Object.keys(changedFields).length > 0) {
                await this.audit.log((0, product_audit_1.buildUpdateProductAudit)({
                    companyId: (0, shop_scope_1.requireCompanyId)(user),
                    userId: user.id,
    ...
    async deletionImpact(user, id) {
        ...
        const [goodsReceiptCount, goodsIssueCount, purchaseOrderCount, damagedCount, ledgerCount] = relatedUsage;
        const currentStock = Array.from(stockBalances.entries())
            .filter(([key]) => key.startsWith(`${id}:`))
            .reduce((sum, [, balance]) => sum + balance, 0);
        const historyCount = goodsReceiptCount + goodsIssueCount + purchaseOrderCount + damagedCount + ledgerCount;
    ...
    async remove(user, id) {
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            await this.audit.log((0, product_audit_1.buildDeleteProductAudit)({
    ...
    async bulkUpdateInventory(user, dto) {
        const errors = [];
        let updated = 0;
    ...
    async applyBulkInventoryRow(tx, user, row, productByCode, shopByNumber) {
        ...
        const nextMax = row.maxStock !== undefined
            ? row.maxStock
        ...
        await tx.productPlant.update({
            where: { id: plant.id },
            data: {
                ...(row.minStock !== undefined ? { minStockLevel: new client_1.Prisma.Decimal(row.minStock) } : {}),
                ...(row.maxStock !== undefined ? { maxStockLevel: new client_1.Prisma.Decimal(row.maxStock) } : {}),
                ...(row.reorderQty !== undefined ? { reorderQty: new client_1.Prisma.Decimal(row.reorderQty) } : {}),
    ...
    async bulkUpsert(user, dto) {
        ...
        const explicitCodes = [
            ...new Set(dto.rows.map((row) => row.productCode?.trim().toUpperCase()).filter(Boolean)),
        ];
        return this.prisma.$transaction(async (tx) => {
            ...
            const reservedCodes = new Set([...explicitCodes, ...existingProducts.map((product) => product.productCode)].map((value) => value.trim().toUpperCase()));
            const locationByKey = new Map(locations.map((location) => [
            ...
            const stockBalances = await this.buildStockBalanceMap(tx, existingProducts.map((product) => product.id), accessibleShops.map((shop) => shop.id));
            let created = 0;
            let updated = 0;
            let validated = 0;
            let failed = 0;
            ...
            for (let index = 0; index < dto.rows.length; index += 1) {
                ...
                try {
                    const result = await this.applyBulkUpsertRow(tx, user, row, rowNumber, validateOnly, productByCode, reservedCodes, accessibleShops, locationByKey, locationsByShop, stockBalances);
                    results.push(result);
    ...
    async applyBulkUpsertRow(tx, user, row, rowNumber, validateOnly, productByCode, reservedCodes, shops, locationByKey, locationsByShop, stockBalances) {
        ...
        const companyId = (0, shop_scope_1.requireCompanyId)(user);
        const productCode = incomingCode ??
        ...
        if (row.storageLocationCode) {
            ...
            if (!location || !location.isActive) {
                throw new Error(`Storage location not found for plant ${shop.shopNumber}: ${row.storageLocationCode}`);
            }
            storageLocationId = location.id;
        }
        else if (!existing) {
            if (rowLocations.length === 1) {
                ...
                throw new Error(`Storage location code is required for plant ${shop.shopNumber} because it has multiple active locations`);
            }
        }
        const plantPayload = {
            shopId: shop.id,
        ...
        if (action === 'create') {
            const companyId = (0, shop_scope_1.requireCompanyId)(user);
            await this.subscriptions.assertSkuLimit(companyId);
            warnings.push(incomingCode ? 'SKU was not found and will be created as a new product.' : 'SKU will be auto-generated from the category.');
            if (!validateOnly) {
                const created = await tx.product.create({
                    data: {
                        ...
                        taxPreference: row.taxPreference ?? client_1.TaxPreference.TAXABLE,
                        purchasePrice: new client_1.Prisma.Decimal(row.purchasePrice),
                        sellingPrice: new client_1.Prisma.Decimal(row.sellingPrice),
                        isActive: row.isActive ?? true,
                        createdById: user.id,
                        plants: {
                            create: {
                                shopId: shop.id,
                                storageLocationId,
                                openingStock: new client_1.Prisma.Decimal(row.openingStock ?? 0),
                                ...this.plantBatchExpiryFields(row),
                                minStockLevel: new client_1.Prisma.Decimal(row.minStockLevel ?? 0),
                                maxStockLevel: row.maxStockLevel === undefined ? null : new client_1.Prisma.Decimal(row.maxStockLevel),
                                reorderQty: row.reorderQty === undefined ? null : new client_1.Prisma.Decimal(row.reorderQty),
                                isActive: row.isActive ?? true,
                                createdById: user.id,
                            },
                        },
                    ...
                    plants: [
                        {
                            ...
                            storageLocation: storageLocationId
                                ? {
                                    ...
                                    code: rowLocations.find((location) => location.id === storageLocationId)?.code ?? '',
                                    name: rowLocations.find((location) => location.id === storageLocationId)?.name ?? '',
                                }
                                : null,
                        },
                    ],
        ...
        const resolvedStorageLocationId = storageLocationId ?? existingPlant?.storageLocationId ?? (rowLocations.length === 1 ? rowLocations[0].id : null);
        ...
        if (Number(row.openingStock ?? 0) !== currentStock) {
            warnings.push(`Current stock will be synchronized from ${currentStock} to ${Number(row.openingStock ?? 0)}.`);
        }
        if (!validateOnly) {
        ...
        if (existingPlant) {
            ...
            existingPlant.openingStock = new client_1.Prisma.Decimal(row.openingStock ?? 0);
            existingPlant.minStockLevel = new client_1.Prisma.Decimal(row.minStockLevel ?? 0);
            existingPlant.maxStockLevel =
                row.maxStockLevel === undefined ? null : new client_1.Prisma.Decimal(row.maxStockLevel);
            existingPlant.reorderQty =
                row.reorderQty === undefined ? null : new client_1.Prisma.Decimal(row.reorderQty);
            ...
            existingProduct.plants.push({
                ...
                storageLocation: resolvedStorageLocationId
                    ? {
                        ...
                        code: rowLocations.find((location) => location.id === resolvedStorageLocationId)?.code ?? '',
                        name: rowLocations.find((location) => location.id === resolvedStorageLocationId)?.name ?? '',
                    }
                    : null,
            });
        }
    ...
    async syncImportedStockLevel(tx, user, args) {
        const delta = Number(args.targetStock) - Number(args.currentStock);
        if (delta === 0) {
            return;
        }
        await this.stock.postMovement(tx, {
            type: delta > 0 ? client_1.TransactionType.OPENING : client_1.TransactionType.GOODS_ISSUE,
