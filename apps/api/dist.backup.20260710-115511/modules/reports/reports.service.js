...
let ReportsService = class ReportsService {
    ...
    subscriptions;
    stock;
    constructor(prisma, subscriptions, stock) {
        this.prisma = prisma;
        this.subscriptions = subscriptions;
        this.stock = stock;
    }
    async assertReportsAllowed(user) {
    ...
    async resolveShopIds(user, shopId) {
        if (shopId) {
    ...
    shopWhere(user, shopId) {
        if (shopId)
            (0, shop_scope_1.assertShopScope)(user, shopId);
        if (shopId) {
            return { ...(0, shop_scope_1.shopListWhere)(user), id: shopId };
        }
        return (0, shop_scope_1.shopListWhere)(user);
    }
    async inventory(user, filters) {
        ...
        return assignments
            ...
            shop_id: a.shopId,
            current_stock: new client_1.Prisma.Decimal(stockByKey.get(`${a.productId}:${a.shopId}`) ?? 0),
    ...
    async lowStock(user, shop_id, category) {
    ...
    async analyticsOverview(user, filters) {
        await this.assertReportsAllowed(user);
        ...
        const shopArray = client_1.Prisma.sql `ARRAY[${client_1.Prisma.join(shopIds.map((id) => client_1.Prisma.sql `${id}::uuid`))}]::uuid[]`;
        const stockQty = client_1.Prisma.raw(`(${(0, effective_current_stock_1.effectiveCurrentStockExpr)('ss')})`);
        const unitCost = client_1.Prisma.raw('COALESCE(NULLIF(ss.avg_cost, 0), NULLIF(p.purchase_price, 0), 0)');
      ...
      `),
            ...
            this.prisma.salesOrderHeader.aggregate({
                ...
                where: {
                    ...
                    status: { not: client_1.SalesOrderStatus.CANCELLED },
                },
            }),
    ...
    async purchaseOrderSummary(user, filters) {
        await this.assertReportsAllowed(user);
        const shopIds = await this.resolveShopIds(user, filters.shop_id);
        const { from, to } = this.resolveDateRange(filters.date_from, filters.date_to);
        const page = Math.max(filters.page ?? 1, 1);
        const limit = Math.min(Math.max(filters.limit ?? 50, 1), 200);
        const statusFilter = Object.values(client_1.PurchaseOrderStatus).includes(filters.status)
            ? filters.status
            : undefined;
        const where = {
            shopId: { in: shopIds },
        ...
        return {
            kpis: {
                ...
                confirmedCount: statusCounts[client_1.PurchaseOrderStatus.CONFIRMED] ?? 0,
                draftCount: statusCounts[client_1.PurchaseOrderStatus.DRAFT] ?? 0,
                cancelledCount: statusCounts[client_1.PurchaseOrderStatus.CANCELLED] ?? 0,
            },
            rows: rows.map((row) => ({
                id: row.id,
    ...
    async salesOrderSummary(user, filters) {
        await this.assertReportsAllowed(user);
        const shopIds = await this.resolveShopIds(user, filters.shop_id);
        const { from, to } = this.resolveDateRange(filters.date_from, filters.date_to);
        const page = Math.max(filters.page ?? 1, 1);
        const limit = Math.min(Math.max(filters.limit ?? 50, 1), 200);
        const statusFilter = Object.values(client_1.SalesOrderStatus).includes(filters.status)
            ? filters.status
            : undefined;
        const where = {
            shopId: { in: shopIds },
        ...
        return {
            kpis: {
                totalCount: totals._count._all,
                totalValue: Number(totals._sum.totalValue ?? 0),
                confirmedCount: statusCounts[client_1.SalesOrderStatus.CONFIRMED] ?? 0,
                fulfilledCount: statusCounts[client_1.SalesOrderStatus.FULFILLED] ?? 0,
                cancelledCount: statusCounts[client_1.SalesOrderStatus.CANCELLED] ?? 0,
            },
            rows: rows.map((row) => ({
                id: row.id,
    ...
    async listSavedFilters(user, reportType) {
        await this.assertReportsAllowed(user);
    ...
    async createSavedFilter(user, payload) {
        await this.assertReportsAllowed(user);
    ...
    async deleteSavedFilter(user, id) {
        await this.assertReportsAllowed(user);
        return this.prisma.reportSavedFilter.deleteMany({
            where: { id, userId: user.id },
        });
    }
    async sumStockValue(shopIds) {
        const shopArray = client_1.Prisma.sql `ARRAY[${client_1.Prisma.join(shopIds.map((id) => client_1.Prisma.sql `${id}::uuid`))}]::uuid[]`;
        const stockQty = client_1.Prisma.raw(`(${(0, effective_current_stock_1.effectiveCurrentStockExpr)('ss')})`);
        const unitCost = client_1.Prisma.raw('COALESCE(NULLIF(ss.avg_cost, 0), NULLIF(p.purchase_price, 0), 0)');
    ...
    computeChangePct(current, prior) {
        if (!prior)
            return null;
        return Math.round(((current - prior) / prior) * 1000) / 10;
    }
    async periodSalesMetrics(shopIds, from, to) {
    ...
    async openPoValue(shopIds, from, to) {
        const shopArray = client_1.Prisma.sql `ARRAY[${client_1.Prisma.join(shopIds.map((id) => client_1.Prisma.sql `${id}::uuid`))}]::uuid[]`;
        const rows = await this.prisma.$queryRaw(client_1.Prisma.sql `
    ...
    async executiveSummary(user, filters) {
        await this.assertReportsAllowed(user);
        const shopIds = await this.resolveShopIds(user, filters.shop_id);
        const { from, to } = this.resolveDateRange(filters.date_from, filters.date_to);
        ...
        const [stockValue, lowStockCount, currentMetrics, priorMetrics, openPo, openPoPrior] = await Promise.all([
        ...
        const pendingOutbox = await this.prisma.documentEmailOutbox.count({
            where: {
                status: { in: [client_1.DocumentEmailStatus.PENDING_PDF, client_1.DocumentEmailStatus.PENDING_SEND] },
            },
        });
        ...
        const staleDrafts = await this.prisma.purchaseOrderHeader.count({
            where: { status: client_1.PurchaseOrderStatus.DRAFT, createdAt: { lt: staleDraftCutoff } },
        });
        const systemHealth = {
            negativeStockCount,
    ...
    async inventoryAging(user, filters) {
        await this.assertReportsAllowed(user);
        const shopIds = await this.resolveShopIds(user, filters.shop_id);
        const shopArray = client_1.Prisma.sql `ARRAY[${client_1.Prisma.join(shopIds.map((id) => client_1.Prisma.sql `${id}::uuid`))}]::uuid[]`;
        const stockQty = client_1.Prisma.raw(`(${(0, effective_current_stock_1.effectiveCurrentStockExpr)('ss')})`);
        const lastMovementAt = client_1.Prisma.raw(`
COALESCE(
  ss.last_movement_at,
  (
  ...
  pp.created_at
        ...
        const unitCost = client_1.Prisma.raw('COALESCE(NULLIF(ss.avg_cost, 0), NULLIF(p.purchase_price, 0), 0)');
        const bucketRows = await this.prisma.$queryRaw(client_1.Prisma.sql `
      WITH base AS (
        SELECT
          pp.shop_id,
          p.id,
    ...
    `);
        ...
        let rows = [];
        if (filters.bucket) {
            rows = await this.prisma.$queryRaw(client_1.Prisma.sql `
          WITH base AS (
            SELECT
              pp.shop_id,
              p.product_code,
    ...
    async grRegister(user, filters) {
        await this.assertReportsAllowed(user);
        const shopWhere = this.shopWhere(user, filters.shop_id);
        const { from, to } = this.resolveDateRange(filters.date_from, filters.date_to);
        const statusFilter = Object.values(client_1.DocumentStatus).includes(filters.status)
            ? filters.status
            : undefined;
    ...
    async shopSummary(user, filters) {
        await this.assertReportsAllowed(user);
        const shopIds = await this.resolveShopIds(user, filters.shop_id);
        const { from, to } = this.resolveDateRange(filters.date_from, filters.date_to);
        const shopArray = client_1.Prisma.sql `ARRAY[${client_1.Prisma.join(shopIds.map((id) => client_1.Prisma.sql `${id}::uuid`))}]::uuid[]`;
    ...
    async deadStock(user, filters) {
      ...
      ${filters.category ? client_1.Prisma.sql `AND p.category = ${filters.category}` : client_1.Prisma.empty}
      ${filters.supplier ? client_1.Prisma.sql `AND s.name ILIKE ${'%' + filters.supplier + '%'}` : client_1.Prisma.empty}
    ...
    `;
      ...
      `),
        ...
        WHERE ${whereClause}
        ORDER BY ${sortBy === 'daysUnsold' ? client_1.Prisma.sql `days_unsold DESC` : client_1.Prisma.sql `stock_value DESC`}
        OFFSET ${(page - 1) * limit}
        LIMIT ${limit}
      `),
        ]);
        ...
        return {
            ...
            items: rows.map((r) => ({
                ...
                severity: r.days_unsold >= 180
                    ? 'CRITICAL'
                    : r.days_unsold >= 120
                        ? 'HIGH'
                        : 'MEDIUM',
                recommendation: r.days_unsold >= 180
                    ? 'STOP_REORDER'
                    : r.days_unsold >= 120
                        ? 'OFFER_DISCOUNT'
                        : 'MONITOR',
    ...
    async reorderIntelligence(user, filters) {
        await this.assertReportsAllowed(user);
        (0, shop_scope_1.assertShopScope)(user, filters.shop_id);
        const now = new Date();
      ...
      `),
        ...
        AND pp.is_active = true
        ${filters.category ? client_1.Prisma.sql `AND p.category = ${filters.category}` : client_1.Prisma.empty}
        ORDER BY ${sortBy === 'daysLeft'
                ? client_1.Prisma.sql `days_remaining ASC`
                : sortBy === 'avgSalesPerDay'
                    ? client_1.Prisma.sql `avg_sales_per_day DESC`
                    :
                        client_1.Prisma.sql `days_remaining ASC`}
        OFFSET ${(page - 1) * limit}
        LIMIT ${limit}
    ...
    async customerAging(user, filters) {
        await this.assertReportsAllowed(user);
        const shopIds = await this.resolveShopIds(user, filters.shop_id);
        const shopArray = client_1.Prisma.sql `ARRAY[${client_1.Prisma.join(shopIds.map((id) => client_1.Prisma.sql `${id}::uuid`))}]::uuid[]`;
        const page = Math.max(filters.page ?? 1, 1);
        ...
        SELECT COUNT(*)::bigint as count FROM (
          ...
          ${filters.customer_name ? client_1.Prisma.sql `AND c.customer_name ILIKE ${'%' + filters.customer_name + '%'}` : client_1.Prisma.empty}
          GROUP BY c.id
      ...
      `),
        ...
        ${filters.show_overdue_only ? client_1.Prisma.sql `WHERE overdue_amount > 0` : client_1.Prisma.empty}
        ORDER BY ${sortBy === 'riskScore'
                ? client_1.Prisma.sql `(CASE WHEN overdue_amount::numeric / NULLIF(total_outstanding::numeric, 0) > 0.5 THEN 1 WHEN overdue_amount > 0 THEN 2 ELSE 3 END) ASC, overdue_amount DESC`
                : sortBy === 'overdueAmount'
                    ? client_1.Prisma.sql `overdue_amount DESC`
                    : client_1.Prisma.sql `total_outstanding DESC`}
        OFFSET ${(page - 1) * limit}
        LIMIT ${limit}
      `),
            this.prisma.$queryRaw(client_1.Prisma.sql `
      ...
      `),
        ...
        return {
            ...
            items: rows.map((r) => {
                ...
                const actions = riskScore === 'CRITICAL'
    ...
    async productProfitability(user, filters) {
        await this.assertReportsAllowed(user);
        const shopIds = await this.resolveShopIds(user, filters.shop_id);
        const shopArray = client_1.Prisma.sql `ARRAY[${client_1.Prisma.join(shopIds.map((id) => client_1.Prisma.sql `${id}::uuid`))}]::uuid[]`;
        const now = new Date();
      ...
      `),
        ...
        HAVING SUM(COALESCE(soi.quantity, 0)) > 0
        ${filters.show_loss_only ? client_1.Prisma.sql `AND (SUM(soi.quantity * soi.unit_price) - (SUM(soi.quantity) * COALESCE(ss.avg_cost, p.purchase_price, 0))) < 0` : client_1.Prisma.empty}
        ORDER BY ${sortBy === 'margin'
                ? client_1.Prisma.sql `margin_percentage DESC`
                : sortBy === 'revenue'
                    ? client_1.Prisma.sql `revenue DESC`
                    : client_1.Prisma.sql `profit DESC`}
        OFFSET ${(page - 1) * limit}
        LIMIT ${limit}
      `),
            this.prisma.$queryRaw(client_1.Prisma.sql `
    ...
    async actionCenter(user, filters) {
        await this.assertReportsAllowed(user);
        const shopIds = await this.resolveShopIds(user, filters.shop_id);
        const actions = [];
