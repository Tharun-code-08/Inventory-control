...
exports.EMAIL_TEMPLATE_DEFINITIONS = exports.EMAIL_TEMPLATE_IDS = exports.EMAIL_NOTIFICATIONS_CONFIG_PREFIX = void 0;
exports.buildDefaultEmailNotificationsConfig = buildDefaultEmailNotificationsConfig;
exports.getTemplateDefinition = getTemplateDefinition;
exports.EMAIL_NOTIFICATIONS_CONFIG_PREFIX = 'email_notifications_config_v1';
...
exports.EMAIL_TEMPLATE_DEFINITIONS = [
    {
        id: 'rfq_invite',
        label: 'RFQ Invite',
        group: 'procurement',
        ...
        defaultBodyText: 'Hello {{supplier_name}},\n\nYou are invited to submit a quotation for {{rfq_number}} — {{rfq_title}}.\nDeadline: {{deadline}}\nPortal: {{portal_url}}\nAccess code: {{access_code}}\n\nThank you,\n{{company_name}}',
        defaultBodyHtml: '<p>Hello {{supplier_name}},</p><p>You are invited to submit a quotation for <strong>{{rfq_number}}</strong> — {{rfq_title}}.</p><p>Deadline: {{deadline}}<br/>Portal: <a href="{{portal_url}}">{{portal_url}}</a><br/>Access code: {{access_code}}</p><p>Thank you,<br/>{{company_name}}</p>',
    },
    {
        ...
        defaultBodyHtml: '<p>Hello {{supplier_name}},</p><p>Please find purchase order <strong>{{po_number}}</strong> dated {{po_date}} for {{shop_name}}.</p><p>Total: {{total_value}}</p><p>Thank you,<br/>{{company_name}}</p>',
    },
    {
    ...
    },
    {
        ...
        defaultBodyHtml: '<p>Hello {{supplier_name}},</p><p>A goods return <strong>{{return_number}}</strong> has been submitted against GR {{gr_number}}.</p><p>{{company_name}}</p>',
    },
    {
        id: 'invoice_created',
        ...
        defaultBodyHtml: '<p>Dear {{customer_name}},</p><p>Invoice <strong>{{invoice_number}}</strong> dated {{invoice_date}} has been raised.</p><p>Due date: {{due_date}}<br/>Amount: {{total_amount}}</p><p>Thank you,<br/>{{company_name}}</p>',
    },
    {
        id: 'payment_received',
        ...
        defaultBodyHtml: '<p>Dear {{customer_name}},</p><p>We received {{payment_type}} payment of {{amount_paid}} against invoice {{invoice_number}}.</p><p>Receipt: {{receipt_number}}<br/>Balance due: {{balance_due}}</p><p>Thank you,<br/>{{company_name}}</p>',
    },
    {
        ...
        defaultBodyHtml: '<p>Dear {{customer_name}},</p><p>Please find sales order <strong>{{so_number}}</strong> dated {{order_date}}.</p><p>Expected date: {{expected_date}}<br/>Plant: {{shop_name}}<br/>Total: {{total_amount}}</p><p>Thank you,<br/>{{company_name}}</p>',
    },
    {
        ...
        defaultBodyHtml: '<p>Hello {{supplier_name}},</p><p>Supplier bill <strong>{{bill_number}}</strong> dated {{bill_date}} has been issued.</p><p>Due date: {{due_date}}<br/>Purchase order: {{po_number}}<br/>Amount: {{total_amount}}</p><p>{{company_name}}</p>',
    },
    {
        ...
        defaultBodyHtml: '<p>Hello {{supplier_name}},</p><p>We recorded {{payment_type}} payment of {{amount_paid}} against bill {{bill_number}}.</p><p>Payment ref: {{payment_number}}<br/>Balance due: {{balance_due}}</p><p>{{company_name}}</p>',
    },
    {
        id: 'payment_reminder',
        ...
        defaultBodyHtml: '<p>Dear {{customer_name}},</p><p>This is a reminder that invoice <strong>{{invoice_number}}</strong> is due in {{days_until_due}} day(s) on {{due_date}}.</p><p>Balance due: {{balance_due}}</p><p>Thank you,<br/>{{company_name}}</p>',
    },
    {
        id: 'user_invite',
        ...
        defaultBodyText: 'Hello {{invitee_name}},\n\n{{inviter_name}} invited you to join {{company_name}} on SoftdigitIMS.\nRole: {{role_name}}\nAccept: {{invite_url}}\n\nSoftdigit Consulting',
        defaultBodyHtml: '<p>Hello {{invitee_name}},</p><p>{{inviter_name}} invited you to join <strong>{{company_name}}</strong> on SoftdigitIMS.</p><p>Role: {{role_name}}<br/><a href="{{invite_url}}">Accept invitation</a></p>',
    },
    {
        ...
        defaultBodyText: 'Goods receipt {{gr_number}} from {{supplier_name}} has been posted for {{shop_name}}.\n\n{{company_name}}',
        defaultBodyHtml: '<p>Goods receipt <strong>{{gr_number}}</strong> from {{supplier_name}} has been posted for {{shop_name}}.</p>',
    },
    {
        ...
        defaultBodyHtml: '<p>Hello {{supplier_name}},</p><p>Goods receipt <strong>{{gr_number}}</strong> dated {{gr_date}} has been posted for {{shop_name}}.</p><p>Purchase order: {{po_number}}<br/>Total: {{total_value}}</p><p>{{company_name}}</p>',
    },
];
function buildDefaultEmailNotificationsConfig() {
    const templates = {};
    for (const def of exports.EMAIL_TEMPLATE_DEFINITIONS) {
        templates[def.id] = {
            enabled: true,
...
function getTemplateDefinition(templateId) {
    const def = exports.EMAIL_TEMPLATE_DEFINITIONS.find((row) => row.id === templateId);
    if (!def)
        throw new Error(`Unknown template: ${templateId}`);
    return def;
}
//# sourceMappingURL=email-notifications.constants.js.map