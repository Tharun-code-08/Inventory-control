...
exports.salesQuotationSubject = salesQuotationSubject;
exports.salesQuotationPortalCtaHtml = salesQuotationPortalCtaHtml;
exports.ensureSalesQuotationPortalCta = ensureSalesQuotationPortalCta;
exports.ensureSalesQuotationPortalText = ensureSalesQuotationPortalText;
exports.salesQuotationText = salesQuotationText;
exports.salesQuotationHtml = salesQuotationHtml;
function salesQuotationSubject(content) {
    return `Sales Quotation ${content.quoteNumber} — ${content.companyName}`;
}
function salesQuotationPortalCtaHtml(portalUrl, isRevision = false) {
...
function ensureSalesQuotationPortalCta(html, portalUrl, isRevision = false) {
...
function ensureSalesQuotationPortalText(text, portalUrl) {
    const url = portalUrl?.trim();
    if (!url || text.includes(url))
        return text;
    return `${text.replace(/\s*$/, '')}\n\nReview and respond online:\n${url}\n`;
}
function salesQuotationText(content) {
    const lines = [
        ...
        '',
        'Line items:',
        ...content.lines.map((l) => `  • ${l.code} — ${l.description}: ${l.quantity} ${l.uom} @ ${l.unitPrice} = ${l.lineValue}`),
        '',
...
function salesQuotationHtml(content) {
...
//# sourceMappingURL=sales-quotation.template.js.map